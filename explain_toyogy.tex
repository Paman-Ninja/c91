...
この設定では奇数ページと偶数ページの左マージンから1inchを引いた値をそれぞれ同一にしています。
...
% 本当に?要検証
% layout.styが示すレイアウトと実際のレイアウトが異なっている可能性が見えたので
% geometryパッケージを[dvipdfm, pass]で実行してlogを見ないと正確なことが言えない
...
つまり,\jyquote{本文}が通常よりもだいぶ広がっています。
このようにして,\jyquote{本文}のレイアウトを確認することができました。

ページレイアウトの設定はまだ続きます。
``本文''下部の余白には``フッター''が挿入されます。
``本文''下端と``フッター''上端のアキについて指定するためには少しだけ考えねばならないことがあります。
\reflisting{list:layout}の10行目に指定されている\verb|\footskip|は
...
\reflisting{list:layout}の設定のなかで異質なものが3行目のマクロです。
...
\reflisting{list:footer}の1行目から9行目が``フッター''にノンブルを表示するための設定です。
...
\reflisting{list:footer}の10行目で\verb|plain|のページスタイルを\verb|\ps@plainfoot|に設定しています。
